...
pickerDateRange.prototype.fillDate = function(year, month, index) {
	...
	if(isTaTheme){
		...
		if(this.mOpts.magicSelect){
			...
			yh.push('</ul></div></div>');
			mh.push('</ul></div></div>');			
			$(cap).append(yh.join('') +'<span class="joinLine"> Năm </span>'+mh.join('')+'<span class="joinLine"> Tháng </span>');

		}
		else{
			$(cap).append(year + 'Năm' + (month + 1) + 'Tháng');
		}
		
		$(table).append(cap);
		thead = document.createElement('thead');
		tr = document.createElement('tr');
		var days = ['VII', 'I', 'II', 'III', 'IV', 'V', 'VI'];
		for(var i = 0; i < 7; i ++) {
	...
	//    $(td).append('<span style="font-size:16px">' + year + '年' + (month + 1) + '月' + '</span>');
		...
		$(td).append(year + 'Năm' + (month + 1) + 'Tháng');
		$(td).attr('colSpan', 7);
		$(td).css('text-align', 'center');
