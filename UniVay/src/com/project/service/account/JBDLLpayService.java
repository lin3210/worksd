...
public class JBDLLpayService  extends BaseService{
	...
	public boolean updateUserRz(String  userid,String  money,String orderid ,DataRow row1 ,DataRow row2)
	{
	  ...
	  public Integer getSHJJQX(int cmsuserid) {
			StringBuffer sb = new StringBuffer();
			sb.append("select sh_jjqx from sdcms_user  where user_id = " + cmsuserid);
	  ...
	  public Integer getUserMark(int  userid)
	  {
		  String sql = "select user_mark from sd_user where id=" +userid;
	  ...
	  public Integer  getHongbaoid(String userid){
		  StringBuffer sb = new StringBuffer();
		  sb.append("select hongbao  from sd_user where  id= "+userid );
		  
		  return getJdbcTemplate().queryInt(sb.toString());
	  }
	  public Integer  getAppflyer(String userid){
		  StringBuffer sb = new StringBuffer();
		  sb.append("select appflyer  from sd_user where  id= "+userid );
		  
		  return getJdbcTemplate().queryInt(sb.toString());
	  }
	  public Integer  getHongbaoidJK(String rec_id){
		  StringBuffer sb = new StringBuffer();
		  sb.append("select hongbaoid  from sd_new_jkyx where  id= "+rec_id );
		  
		  return getJdbcTemplate().queryInt(sb.toString());
	  }
	  public String  getCHGResult(String rec_id){
			StringBuffer sb = new StringBuffer();
			sb.append("select sfyfk  from sd_new_jkyx where  id= '"+rec_id+"'" );
			return getJdbcTemplate().queryString(sb.toString());
		}
	  public Integer  getSBResult(String rec_id){
			StringBuffer sb = new StringBuffer();
			sb.append("select fkr  from sd_new_jkyx where  id= '"+rec_id+"'" );
			return getJdbcTemplate().queryInt(sb.toString());
		}
	  ...
	  public String getSjshsh(String jkid){
			StringBuffer sb = new StringBuffer();
	  ...
	  public String getLX(String jkid){
		  StringBuffer sb = new StringBuffer();
		  sb.append("select lx from sd_new_jkyx  where id='"+jkid+"'" );
		  return getJdbcTemplate().queryString(sb.toString());
	  }
	  public String getSjdzYN(int jkid){
			StringBuffer sb = new StringBuffer();
	  ...
	  public int getAllFKcount(String userid,String timeday){
			String sql = "select count(id) from sd_withdraw where userid="+userid+" and substring(checktime,1,10)='"+timeday+"' ";
			return getJdbcTemplate().queryInt(sql);
		}
   ...
   /*   public void updateUserHongbao(DataRow row) throws Exception {
    	  
    	  getJdbcTemplate().update("sd_hongbao", row, "hongbaoid", row.getString("hongbaoid"));
      }*/
      public Integer getOLDYH(String  userid)
	  {
		  String sql = "select oldyonghu from sd_user where id= '" +userid+"' " ;
