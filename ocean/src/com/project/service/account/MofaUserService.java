...
public class MofaUserService extends BaseService
{
	private static Logger logger = Logger.getLogger(MofaUserService.class);
	...
	public void insertIpInfo(DataRow data)
	 ...
	 public Integer getDxtiaoshu(String phone  ,String dateTS ,String dateTE ){
			...
			sb.append("select count(*) from sd_ip  where createTime >'"+dateTS+"'and createTime <'"+dateTE +"'  and phone='"+phone+"'");
    ...
    */
	...
	public void insertUserhongbao(DataRow data)
	{
     ...
     public List<DataRow> getShowJD(int type){
	     String sql ="select sd_new_jkyx.jk_money as jk_money , sd_new_jkyx.create_date as create_date ,sd_bankcard.cardUsername as cardUsername from sd_new_jkyx  left join  sd_bankcard on sd_bankcard.userId = sd_new_jkyx.userid   where cl_status = "+ type+" order by create_date desc limit 10"  ;
		 List<DataRow> list = getJdbcTemplate().query(sql);			
		 return list;
	}
     public DBPage getRecordJRList(int curPage, int numPerPage,  String startDate, String endDate,
 			 String hkstat) throws Exception {
     ...
     public DataRow getRZ(int userid) {
 		String sql = "select * from sd_user_finance a join sd_bankcard b ON a.userId = b.userId where a.userId=" + userid ;
 		return getJdbcTemplate().queryMap(sql);
 	}
     public DataRow getALLRZ(int userid) {
    	 String sql = "select isshenfen,isjop,islianxi,yhbd,isschool,profession from sd_user_finance a join sd_user b ON a.userId = b.Id where a.userId=" + userid ;
    	 return getJdbcTemplate().queryMap(sql);
     }
     ...
     public Integer getCountPhone(String phone){
    	 StringBuffer sb = new StringBuffer();
    	 sb.append("select count(1) from sd_zmrz where phonenumber = ");
    	 sb.append(phone);
    	 return getJdbcTemplate().queryInt(sb.toString());
     }
     public Integer getCountByPhone(String phone){
 		StringBuffer sb = new StringBuffer();
 		sb.append("select count(1) from sd_user where mobilePhone = ");
 		sb.append(phone);
 		return getJdbcTemplate().queryInt(sb.toString());
 	}
 	...
 	{
 		String sql = "select spzt from sd_new_jkyx where cl_status=1 and cl02_status=1 and cl03_status=0 and sfyfk=0 and userid = "+userid;
 		return getJdbcTemplate().queryInt(sql);
 	}
	...
	public void updateUserFinance(String userId,String realName,String idNo,String hint,String phone,String pay_type){
	...
	public void addBankcard(String userId,String cardUserName,String bankName,String cardNo,String commitTime,int cardStatus,String bankbs){
		DataRow data = new DataRow();
	...
	public void addUserFinance(String userId,String realName,String idNo,String cellPhone,int status,String pay_type){
		DataRow data = new DataRow();
		data.set("userId", userId);
		data.set("realName", realName);
		data.set("idNo", idNo);
	...
	public void updateBankcard(String userId,String cardUserName,String bankName,String cardNo,String hint,String  bankbs){
		DataRow data = new DataRow();
		data.set("cardUserName", cardUserName);
		data.set("bankName", bankName);
		data.set("bankbs", bankbs);
...
public DBPage getJKJDListPage(int curPage ,int numPerPage,int userId)throws Exception
	{
	    ...
	    String sql = "select jk_money ,create_date ,jksfwc ,sfyfk, jk_date  ,cl03_status ,cl02_status ,cl_status ,spzt ,sjds_money ,sjsh_money ,id,hkqd from sd_new_jkyx  where userid  = "+userId+"  order by id desc";	
		return getJdbcTemplate().queryPage(sql, curPage, numPerPage);
	}
public DataRow findUserJKByuserid(String id){
		StringBuffer sb = new StringBuffer();
		sb.append("select id , cl_status  ,cl02_status,cl03_status ,spzt,jk_money ,sjsh_money ,sjds_money,yuq_ts , sfyfk, jksfwc  from sd_new_jkyx  where   jksfwc=0 and   cl_status <>3 and cl02_status <>3 and cl03_status <>3  and  userid = ");
		sb.append(id);
...
public DataRow findUserJJJKByuserid(String id){//拒绝借款的信息
	StringBuffer sb = new StringBuffer();
	sb.append("select id , cl_status  ,cl_yj,cl_time,cl02_time,cl03_time,cl02_status,cl03_status ,cl03_yj,spzt,jk_money ,sjsh_money ,sjds_money,yuq_ts , sfyfk, jksfwc  from sd_new_jkyx  where userid = ");
	sb.append(id);
	sb.append(" order by id desc  limit 1 ");
	return getJdbcTemplate().queryMap(sb.toString());
}

public DBPage getTjjlListPage(int curPage ,int numPerPage,int userId, String startDate , String endDate)throws Exception
{
	
    String sql = "select handleSum ,recordTime ,sd_user_finance.realName as realName , fundMode from sd_fundrecord left join sd_user_finance on  sd_fundrecord.trader  = sd_user_finance.userId  where  sd_fundrecord.userId  = "+userId +" and operateType in (46,66,56)  order by recordTime desc";	
...
public DataRow findUserJKById(int jkid){
	...
	String sql = "select sjsh_money , fkdz_time ,hongbaoid,jk_date, hkyq_time ,annualrate ,yuq_ts ,yuq_lx ,yqannualrate ,sjds_money ,sd_user_finance.realname as realName ,sd_bankcard.cardNo ,sd_bankcard.remark as idNo from sd_new_jkyx  left join sd_bankcard on sd_bankcard.userId = sd_new_jkyx.userId left join sd_user_finance on sd_user_finance.userId = sd_new_jkyx.userId where sd_new_jkyx.id = " +jkid;
	return  getJdbcTemplate().queryMap(sql);		
}
public DataRow findUserJKFQById(int jkid){
	
	String sql = "select sjsh_money , fkdz_time ,jk_date, hkfq_time as hkyq_time,hkfq_code ,annualrate ,yuq_ts ,yuq_lx ,yqannualrate ,sjds_money ,sd_user_finance.realname as realName ,sd_bankcard.cardNo ,sd_bankcard.remark as idNo from sd_new_jkyx  left join sd_bankcard on sd_bankcard.userId = sd_new_jkyx.userId left join sd_user_finance on sd_user_finance.userId = sd_new_jkyx.userId where sd_new_jkyx.id = " +jkid;
	return  getJdbcTemplate().queryMap(sql);		
}
 ...
 public DataRow getRepaymentJKMoney(int  userid)
	{
		String sql = "select cardNo, mobilePhone ,cardUserName ,bankbs ,bankName ,remark from sd_user  left join  sd_bankcard on sd_bankcard.userId =sd_user.id where userId = " +userid ;
 ...
 public String getTjliCount(int userid){
		StringBuffer sb = new StringBuffer();
		sb.append("select ifnull(sum(handleSum),0) from sd_fundrecord left join sd_user_finance on  sd_fundrecord.trader  =sd_user_finance.userId  where   operateType in('66','56')  and sd_fundrecord.userId  = " );
		sb.append(userid);
		return getJdbcTemplate().queryString(sb.toString());
	}
 ...
 public List<DataRow> getJKSB1ListOne( int userid)
	{
	   String sql = "select j.id as jkid, u.id as id ,f.realName , INSERT(u.mobilePhone,4,4,'****') ,INSERT(f.idNo,4,4,'****'), j.jk_money, j.create_date ,j.cl_status ,j.cl_yj  ," +
		   		    "j.cl_time ,j.cl_ren from sd_new_jkyx j  left join sd_user u on j.userid = u.id LEFT join sd_user_finance f on f.userId =j.userid  where  1 =1  and j.cl_status =3 and j.userid ="+userid +" order by j.create_date desc limit 4  ";
	  return getJdbcTemplate().query(sql);
	}
...
public List<DataRow> getJKSBListTwo( int userid)
	{
	   String sql = "select j.id as jkid, u.id as id ,f.realName , INSERT(u.mobilePhone,4,4,'****'),INSERT(f.idNo,4,4,'****'), j.jk_money, j.create_date ,j.cl02_status ,j.cl02_yj  ," +
		   		    "j.cl02_time ,j.cl02_ren from sd_new_jkyx j  left join sd_user u on j.userid = u.id LEFT join sd_user_finance f on f.userId =j.userid  where  1 =1 and j.cl02_status =3 and j.userid ="+userid +" order by j.create_date desc limit 4  ";
	  return getJdbcTemplate().query(sql);
	}
...
public List<DataRow> getJKSBListThree( int userid)
	{
	   String sql = "select j.id as jkid, u.id as id ,f.realName , INSERT(u.mobilePhone,4,4,'****') ,INSERT(f.idNo,4,4,'****'), j.jk_money, j.create_date ,j.cl03_status ,j.cl03_yj  ," +
		   		    "j.cl03_time ,j.cl03_ren from sd_new_jkyx j left join sd_user u on j.userid = u.id LEFT join sd_user_finance f on f.userId =j.userid  where  1 =1 and j.cl03_status =3 and j.userid ="+userid +" order by j.create_date desc limit 4  ";
	  return getJdbcTemplate().query(sql);
	}
...
{
	String sql = "select j.fkdz_time, j.cl02_time,j.cl_ren,j.cl_time,j.cl03_time ,j.cl02_ren ,j.cl03_ren ,j.hkyq_time,j.hk_time ,j.hkyq_time,j.sjds_money,j.lx,  j.sjsh_money , j.sfyhw ,j.create_date " +
			" from  sd_new_jkyx j  where  j.sfyfk=1  and j.yuq_ts>0 and j.userid ="+userid +" order by j.create_date desc ";
	return getJdbcTemplate().query(sql);
} 
public List<DataRow> getJKCGWHList( int userid)
{
	String sql = "select j.fkdz_time, j.cl02_time,j.cl_ren,j.cl_time,j.cl03_time ,j.cl02_ren ,j.cl03_ren ,j.hkyq_time,j.hk_time ,j.hkyq_time,j.sjds_money,j.lx,  j.sjsh_money , j.sfyhw ,j.create_date " +
			" from  sd_new_jkyx j  where  j.sfyfk=1 and j.sfyhw=0 and j.yuq_ts>0 and j.userid ="+userid +" order by j.create_date desc ";
	return getJdbcTemplate().query(sql);
} 
...
public Integer  getOLDstate(int userid){
 ...
 public String  getIdno(int userid){
  ...
  public Integer getTotalInvestmentCount(int userid){
	 ...
	 StringBuffer sb = new StringBuffer();
     sb.append("select ifnull(sum(handleSum),0) from  sd_fundrecord  where fundmode='推荐人投资奖励' and userId =" +userid);		
     return getJdbcTemplate().queryInt(sb.toString());
  }
 
  public String getTjjlCount(int userid){
		StringBuffer sb = new StringBuffer();
		sb.append("select ifnull(sum(handleSum),0) from sd_fundrecord left join sd_user_finance on  sd_fundrecord.trader  =sd_user_finance.userId  where   operateType ='66'  and sd_fundrecord.userId  = " );
		sb.append(userid);
		return getJdbcTemplate().queryString(sb.toString());
	}
  public DBPage getPresentationRecordList(int curPage ,int numPerPage,int userId)throws Exception {
	 ...
	 String sql = "select * from sd_withdraw where status <>5 and userId ="+userId +" and remark ='提现' order by applytime desc ";	
   ...
   public String  getPresentationTotal(int userid){
	 ...
	 StringBuffer sb = new StringBuffer();
     sb.append("select ifnull(sum(sum),0) from  sd_withdraw  where status <>5  and remark ='提现' and userId ="+ userid );
     return getJdbcTemplate().queryString(sb.toString());
   }
   ...
   public String  getUserpicVideoZT(int userid,int jkid){
		 ...
		 String sql = "select spzt from sd_new_jkyx where cl_status=1 and cl02_status=1 and cl03_status=0 and sfyfk=0 and userid ="+userid+" and id="+jkid ;	 
		 return getJdbcTemplate().queryString(sql);	 
	 }
   ...
   public DataRow getjkNumLast(String userid){
		String sql = "SELECT yuq_ts,sjds_money,lx,jyfk_money FROM sd_new_jkyx WHERE sfyfk=1 AND sfyhw =1 AND userid="+userid+" order by id desc ";
		return getJdbcTemplate().queryMap(sql);		
	}
   public DataRow getUserRecThreeInfo(int  userid)
	{
		String sql = "select sd_user.id as userid ,sd_bankcard.cardNo, mobilePhone ,username ,sd_user_finance.realname, sd_bankcard.cardUserName ,sd_bankcard.bankbs ,sd_bankcard.bankName ,sd_bankcard.remark from sd_user  left join  sd_bankcard on sd_bankcard.userId =sd_user.id left join  sd_user_finance on sd_user_finance.userId =sd_user.id where sd_bankcard.userId = " +userid;
