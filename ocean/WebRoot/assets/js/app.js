var App = function () {
  ...
  var d = function () {
    ...
    $('#sidebar-content ul > li > a').on('click', function (C) {
      ...
      if ($(window).width() > 767) {
        ...
        B.children('/assets/js/li.open').children('a').children('i.arrow').removeClass(z).addClass(y);
        B.children('/assets/js/li.open').children('.sub-menu').slideUp(200);
        B.children('li.open-default').children('.sub-menu').slideUp(200);
        B.children('/assets/js/li.open').removeClass('open').removeClass('open-default')
      }
      var A = $(this).next();
      if (A.is(':visible')) {
  ...
  return {
    ...
    blockUI: function (x, y) {
      var x = $(x);
      x.block({
        message: '<img src="/assets/img/ajax-loading.gif"/*tpa=http://dsdcms.loserbank.com/assets/js/assets/img/ajax-loading.gif*/ alt="">',
